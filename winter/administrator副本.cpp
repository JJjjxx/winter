...
void administrator_mode(){
	...
	while(true){
		...
		if (opt == "1") { SQL("SELECT * FROM commodity");  ad1.Boss_manu(); continue;}
...
bool Administrator::Good_del() {
	...
	if (y_n == "y") {
		...
		std::string sq = "UPDATE commodity SET 商品状态 = 已下架 WHERE 商品 ID = " + m_id;
		SQL(sq);
		cout << "下架成功！";
		writecommodity();
		return true;
...
bool Administrator::User_del() {
	...
	if (y_n == "y") {
		...
		std::string sq = "UPDATE user SET 用户状态 = 封禁 WHERE 用户ID = " + m_id;
		SQL(sq);
